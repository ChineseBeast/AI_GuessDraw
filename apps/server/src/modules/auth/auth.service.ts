...
export class AuthService {
  ...
  private loadFromFile(): void {
    try {
      ...
      console.warn('[AuthService] 无法加载用户数据，将以空白状态启动:', (err as Error).message);
    }
  }
  ...
  private saveToFile(): void {
    try {
      ...
      const data: PersistedUser[] = [...this.users.values()].map((u) => ({
        id: u.id,
        username: u.username,
        email: u.email,
        avatar: u.avatar,
      ...
      console.error('[AuthService] 保存用户数据失败:', (err as Error).message);
    }
  }
  ...
  getUserById(id: string): PublicUserProfile | null {
    ...
    // 确保旧用户也有 stats 字段
    if (!user.stats) {
      user.stats = { gamesPlayed: 0, gamesWon: 0, totalScore: 0, currentStreak: 0 };
      this.saveToFile();
    }
